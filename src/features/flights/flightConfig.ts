import type { FlightPlan, FlightStatus, JourneyMoment } from './flightTypes'
...
export function getJourneyMoment(statuses:Record<string,FlightStatus|undefined>,now=new Date()):JourneyMoment{
  const nowMs=now.getTime()
  const live=flights.find(f=>{const status=statuses[f.id]?.status;return status&&['boarding','departed','enroute','delayed'].includes(status)})
...
export function formatFlightTime(iso:string|undefined,timeZone:string){return iso?new Intl.DateTimeFormat('de-DE',{timeZone,hour:'2-digit',minute:'2-digit'}).format(new Date(iso)):undefined}
